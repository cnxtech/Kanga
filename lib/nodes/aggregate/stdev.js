...
extend(Stdev, kangaBaseNode);

/**
 * Handle the incoming event
 * @returns {Object}
 */
Stdev.prototype._execute = function () {
	var aggResult = [];
 ...
 * @returns {Array}
 */
Stdev.prototype.aggregate = function () {
	var results = [];
 ...
 * @returns {Array}
 */
Stdev.prototype.getResult = function () {
	var result = [];
	var obj = new Object();
	var root = new Object();
 ...
 * @returns {Boolean}
 */
Stdev.prototype.isEmpty = function (obj) {
	if (obj == null)
		return true;
	for ( var prop in obj) {
 ...
 * @returns {Boolean}
 */
Stdev.prototype.addNewEvent = function () {
	this.eventCount++;
 ...
 */
Stdev.prototype.popEvents = function () {
 ...
 */
Stdev.prototype.eventOnPopped = function (keys, message) {
 ...
 * @param {Object} message
 */
Stdev.prototype.eventOnPushed = function (keys, message) {
	var aggregator = this.aggregators[keys];
	if (!aggregator) {
		aggregator = new StdevImp(this.inputFieldKeys);
 ...
 */
Stdev.prototype.getGroupByKeys = function (message) {
	var key = '';
 ...
 * reset node status
 */
Stdev.prototype.clear = function () {
	this.eventCount = 0;
	this.eventQueue = [];
 ...
 */
var StdevImp = function (aggkeyList) {
 ...
 * @returns {Object}
 */
StdevImp.prototype.aggregate = function () {
	var message = {};
	for (var i = 0; i < this.aggSize; i++) {
		var result = 0.0;
		if (this.countList[i] > 0) {
			var variance = this.powerSumList[i] / this.countList[i]
					- Math.pow((this.sumList[i] / this.countList[i]), 2);
			result = Math.sqrt(variance);
			var keys = this.aggkeyList[i].slice(0, this.aggkeyList[i].length);
			keys[keys.length - 1] = keys[keys.length - 1] + '_stdev';
			putJSONValue(message, keys, result);
		}
	}
	return message;
};
 ...
 */
StdevImp.prototype.reset = function () {
	for (var i = 0; i < this.aggSize; i++) {
		this.sumList[i] = 0.0;
 ...
 * @param {Object} message
 */
StdevImp.prototype.eventOnPush = function (message) {
	for (var i = 0; i < this.aggSize; i++) {
 ...
 */
StdevImp.prototype.eventsOnPopped = function (messageList) {
...
StdevImp.prototype.eventsOnCleared = function () {
	this.reset();
};
 ...
 * @returns {Boolean}
 */
StdevImp.prototype.isEmpty = function () {
	for (var i = 0; i < this.aggSize; i++) {
		if (this.countList[i] > 0)
			return false;
	}
