...
extend(Count, kangaBaseNode);

/**
 * Handle the incoming event
 * @returns {Object}
 */
Count.prototype._execute = function () {
	var aggResult = [];
 ...
 * @returns {Array}
 */
Count.prototype.aggregate = function () {
	var results = [];
 ...
 * @returns {Array}
 */
Count.prototype.getResult = function () {
	var result = [];
	var obj = new Object();
	var root = new Object();
 ...
 * @returns {Boolean}
 */
Count.prototype.isEmpty = function (obj) {
	if (obj == null)
		return true;
	for ( var prop in obj) {
 ...
 * @returns {Boolean}
 */
Count.prototype.addNewEvent = function () {
	this.eventCount++;
 ...
 */
Count.prototype.popEvents = function () {
 ...
 */
Count.prototype.eventOnPopped = function (keys, message) {
 ...
 * @param {Object} message
 */
Count.prototype.eventOnPushed = function (keys, message) {
	var aggregator = this.aggregators[keys];
	if (!aggregator) {
		aggregator = new CountImp();
 ...
 */
Count.prototype.getGroupByKeys = function (message) {
	var key = '';
 ...
 * @returns {Object}
 */
CountImp.prototype.aggregate = function () {
	var message = {};
	if (this.count == 0)
		return null;
 ...
 */
CountImp.prototype.reset = function () {
	this.count = 0;
};
 ...
 * @param {Object} message
 */
CountImp.prototype.eventOnPush = function (message) {
	this.count++;
};
 ...
 */
CountImp.prototype.eventsOnPopped = function (messageList) {
	this.count -= Math.min(this.count, messageList.length);
};

CountImp.prototype.eventsOnCleared = function () {
	this.reset();
};
 ...
 * @returns {Boolean}
 */
CountImp.prototype.isEmpty = function () {
	return this.count <= 0;
};
