...
extend(Sum, kangaBaseNode);

/**
 * Handle the incoming event
 * @returns {Object}
 */
Sum.prototype._execute = function () {
	var aggResult = [];
 ...
 * @returns {Array}
 */
Sum.prototype.aggregate = function () {
	var results = [];
 ...
 * @returns {Boolean}
 */
Sum.prototype.isEmpty = function (obj) {
	if (obj == null)
		return true;
	for ( var prop in obj) {
 ...
 * @returns {Boolean}
 */
Sum.prototype.addNewEvent = function () {
	this.eventCount++;
 ...
 */
Sum.prototype.popEvents = function () {
 ...
 */
Sum.prototype.eventOnPopped = function (keys, message) {
 ...
 * @param {Object} message
 */
Sum.prototype.eventOnPushed = function (keys, message) {
	var aggregator = this.aggregators[keys];
	if (!aggregator) {
		aggregator = new SumImp(this.inputFieldKeys);
 ...
 */
Sum.prototype.getGroupByKeys = function (message) {
	var key = '';
 ...
 */
var SumImp = function (aggkeyList) {
 ...
 */
SumImp.prototype.reset = function () {
	for (var i = 0; i < this.aggSize; i++) {
		this.sumList[i] = 0.0;
		this.countList[i] = 0;
	}
 ...
 * @param {Object} message
 */
SumImp.prototype.eventOnPush = function (message) {
	for (var i = 0; i < this.aggSize; i++) {
 ...
 */
SumImp.prototype.eventsOnPopped = function (messageList) {
...
SumImp.prototype.eventsOnCleared = function () {
	this.reset();
};
 ...
 * @returns {Boolean}
 */
SumImp.prototype.isEmpty = function () {
	for (var i = 0; i < this.aggSize; i++) {
		if (this.countList[i] > 0)
			return false;
	}
