...
def get_windows_service_status(request):
    try:
        if request.method == 'POST':
            ...
            if ServiceExists(servicename):
                ...
                service_configuration = service.Configurations
                service_status = { 'state':str(service_status['CurrentState']),
                                   'servicename':str(service_status['ServiceName']),
                                   'description':str(service_configuration['Description']),
                                   'starttype':str(service_configuration['StartType'])}
        else:
    ...
    return JsonResponse({'result':'success','srv_status':service_status})
...
def service_onoff(request):
    try:
        if request.method == 'POST':
            servicename = request.POST.get('servicename','')
            isstart = ast.literal_eval(request.POST.get('isstart',''))
            if ServiceExists(servicename):
                service = GetService(servicename)
                if isstart:
                    service.Start()
